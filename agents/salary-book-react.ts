...
/**
 * Salary Book (React prototype) Agent
 *
 ...
 *   bun agents/salary-book-react.ts --once
 *   bun agents/salary-book-react.ts --dry-run
 */

loop({
  name: "salary-book-react",
  ...
  run(state) {
    if (state.hasTodos) {
      return work(
        ...
        Create API routes in prototypes/salary-book-react/src/api/routes/salary-book.ts, then fetch from React components.

        IMPORTANT: This agent targets the React prototype in prototypes/salary-book-react/. Paths like src/features/SalaryBook/ mean prototypes/salary-book-react/src/features/SalaryBook/.

        ARCHITECTURE — Follow the barrel pattern (all paths relative to prototypes/salary-book-react/):

        prototypes/salary-book-react/src/features/SalaryBook/
