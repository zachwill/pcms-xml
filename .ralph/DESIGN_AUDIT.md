...
- [x] `web/app/views/entities/agents/_workspace_main.html.erb`: audit for design consistency — ✅ **Fixed:** Bare em-dash `—` nil-value rendering in inline ternaries (agents table L137-139: Expiring/Restrictions/Options columns; agencies table L214: Teams column, L239-241: Expiring/Restrictions/Options columns) was missing `text-muted-foreground` styling. Other columns in the same file already used `<span class="text-muted-foreground">—</span>`. Fixed all 7 instances to use `content_tag(:span, "—", class: "text-muted-foreground")` for consistency. Table headers (`bg-muted/40 text-[10px] uppercase tracking-wide text-muted-foreground/90`), row hover (`hover:bg-yellow-50/70 dark:hover:bg-yellow-900/10 transition-colors duration-75`), `font-mono tabular-nums` on all numeric cells, `entity-cell-two-line` on identity cells, `entity-chip` tokens, and dark mode variants all already matched design guide — no other changes needed.
- [x] `web/app/views/entities/agents/_rightpanel_overlay_agent.html.erb`: audit for consistent sidebar patterns matching Salary Book's `_sidebar_agent.html.erb` — ✅ **Fixed:** (1) Root div changed from `absolute inset-0 bg-background overflow-y-auto border-l border-border` to `h-full` matching Salary Book overlay pattern. (2) Agent header upgraded to include initials avatar (violet gradient) + `<h2 class="text-lg">` matching `_sidebar_agent`. (3) Main container spacing `space-y-4` → `space-y-5`. (4) KPI grid changed from `grid-cols-2` to `grid-cols-4` for client counts. (5) KPI sections wrapped in `border-t border-border pt-4 space-y-2.5` separator. (6) Added `variant` param to 2W KPI cell. (7) Year labels now use `format_year_label()` consistently. (8) Contract mix and Restrictions+options sections changed from card-wrapped (`rounded-lg border`) to flat `border-t` sections with `text-sm` rows, `py-1.5` spacing, and `font-medium` values. (9) Client list upgraded with team logos, headshots, YOS + contract marker meta line, salary total subline, Two-Way/No-salary states, and chevron indicators. (10) Back button styling aligned with Salary Book's `font-medium` "Back" label and `focus-visible` classes. (11) Conditional "Upcoming" section replaces always-visible "Restrictions + options" card. (12) Empty state matches Salary Book's `bg-muted/30 text-center` pattern.
- [x] `web/app/views/entities/agents/_rightpanel_overlay_agency.html.erb`: audit for sidebar consistency — ✅ **Fixed:** (1) Root div changed from `absolute inset-0 bg-background overflow-y-auto border-l border-border` to `h-full` matching Salary Book overlay pattern. (2) Back button aligned: added `font-medium` "Back" label and `focus-visible` classes to match `_rightpanel_overlay_agent`. (3) Agency header upgraded with violet gradient initials avatar + `<h2 class="text-lg">` matching agent overlay. (4) Main container `space-y-4` → `space-y-5`. (5) KPI grid restructured: count row from `grid-cols-2` → `grid-cols-4` (Agents/Clients/Teams/2W), added 2W with muted variant. (6) Added 3-col year book KPIs with `format_year_label()` and percentile subvalues for 2025/2026/2027. (7) KPI sections wrapped in `border-t border-border pt-4 space-y-2.5` separator. (8) Composition section changed from card-wrapped (`rounded-lg border`) to flat `border-t` section with `text-sm` rows, `py-1.5` spacing, and `font-medium` values. (9) Added conditional "Upcoming" section (expiring/options/restrictions) matching agent overlay pattern. (10) Top agents section changed from card-wrapped to flat `border-t` with count header, double-row pattern (name + meta line), salary + percentile right column, and chevrons. (11) Top clients section changed from card-wrapped with `entity-cell-two-line` to flat `border-t` with team logos, headshots, double-row identity cells, Two-Way/No-salary states, total salary subline, and chevrons. (12) Empty states use `bg-muted/30 text-center` pattern. (13) Hover treatment normalized to `hover:bg-muted/50` matching agent overlay client list.
...
- [x] `web/app/views/entities/draft_selections/`: audit all files for design consistency — ✅ **Fixed:** (1) `index.html.erb` L63: `entity-cell-secondary` for transaction ID was missing `font-mono tabular-nums` — added. (2) `index.html.erb` L71: `entity-cell-secondary` for player_id was missing `font-mono tabular-nums` — added. (3) `show.html.erb` L113: provenance table trade_date column was missing `font-mono tabular-nums` — added (matches sibling `draft_picks/show.html.erb` pattern). (4) `show.html.erb` L132-134: transaction/trade ID links in footer were missing `font-mono tabular-nums` — added. All other patterns (shell C, `<thead>` treatment, row hover, KPI cards, empty states, dark mode via theme tokens) already matched design guide — no further changes needed.
