...
class EntitiesPlayersIndexTest < ActionDispatch::IntegrationTest
  ...
  test "players index renders workbench commandbar and urgency surfaces" do
    with_fake_connection do
      ...
      assert_includes response.body, 'id="players-urgency-lane-rail"'
      ...
      assert_includes response.body, 'id="players-urgency-badges"'
      ...
      assert_not_includes response.body, "Compare slots"
    end
  end

  test "players index ignores legacy compare params" do
    with_fake_connection do
      get "/players", params: {
        compare_a: "1",
        compare_b: "2"
      ...
      assert_response :success
      assert_includes response.body, 'id="players-workspace"'
      assert_not_includes response.body, 'id="players-compare-strip"'
  ...
  test "players refresh uses one sse response for multi-region patches" do
    with_fake_connection do
      ...
      assert_not_includes response.body, "players-compare-strip"
    end
  end

  test "players refresh ignores legacy compare action params" do
    with_fake_connection do
      get "/players/sse/refresh", params: {
        q: "",
        team: "ALL",
      ...
      assert_includes response.body, '"overlaytype":"none"'
      assert_includes response.body, '"selectedplayerid":""'
      assert_not_includes response.body, '"comparea":"1"'
      assert_not_includes response.body, '"compareb":""'
      assert_not_includes response.body, "players-compare-strip"
    end
  end
  ...
  test "players sidebar returns simplified overlay pivots and clear endpoint empties overlay" do
    with_fake_connection do
      ...
      assert_not_includes response.body, "compare_action=pin"
      assert_not_includes response.body, "compare_action=clear_slot"
