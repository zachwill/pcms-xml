...
class ToolsTeamSummaryTest < ActionDispatch::IntegrationTest
  ...
  class FakeConnection
    ...
    def exec_query(sql)
      if sql.include?("SELECT DISTINCT salary_year") && sql.include?("FROM pcms.team_salary_warehouse")
        return ActiveRecord::Result.new(["salary_year"], [[2024], [2025]])
      end
    ...
    def columns
      [
        ...
        "roster_row_count", "two_way_row_count", "luxury_tax_owed", "refreshed_at"
      ]
    end
  ...
  test "team summary renders compare strip and sidebar surfaces" do
    with_fake_connection do
      ...
      assert_includes response.body, "Compare actions"
    end
  end
