...
function getCapAmount(row: DeadMoney, year: (typeof SALARY_YEARS)[number]): number | null {
  switch (year) {
    case 2025:
      return row.cap_2025;
    case 2026:
...
export function DeadMoneySection({ deadMoney }: { deadMoney: DeadMoney[] }) {
  if (deadMoney.length === 0) return null;

  return (
    <div>
      <SectionHeader label="Dead Money" count={deadMoney.length} />

      {deadMoney.map((row) => {
        const title = row.player_name ?? "Dead Money";
        const subtitle = row.waive_date ? `Waived ${row.waive_date}` : null;
...
export default DeadMoneySection;
