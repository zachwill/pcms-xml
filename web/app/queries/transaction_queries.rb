class TransactionQueries
  ...
  def fetch_ledger_entries(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_draft_selection(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_trade_summary(trade_id)
    trade_sql = conn.quote(trade_id)
  ...
  def fetch_trade_transactions(trade_id)
    trade_sql = conn.quote(trade_id)
  ...
  def fetch_cap_exception_usage_rows(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_cap_dead_money_rows(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_cap_budget_snapshot_rows(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_index_transactions(where_sql:)
    sql_where = where_sql.to_s.strip.presence || "1=1"
  ...
  def fetch_sidebar_transaction(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_sidebar_ledger_summary(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_sidebar_artifact_summary(transaction_id)
    id_sql = conn.quote(transaction_id)
  ...
  def fetch_sidebar_trade_summary(trade_id)
    trade_sql = conn.quote(trade_id)
