class PlayerQueries
  ...
  def fetch_player_name_for_slug_seed(player_id)
  ...
  def fetch_index_players(where_sql:, sort_sql:, limit:, option_presence_sql:, non_guaranteed_presence_sql:, next_option_sql:, next_non_guaranteed_sql:, lock_now_sql:, expiring_sql:, horizon_cap_column:, next_cap_column:)
    conn.exec_query(<<~SQL).to_a
      SELECT
        sbw.player_id,
        sbw.player_name,
  ...
  def fetch_player_header(player_id)
    id_sql = conn.quote(player_id)
  ...
  def fetch_salary_book_row(player_id)
    id_sql = conn.quote(player_id)
  ...
  def fetch_draft_selection(player_id)
    id_sql = conn.quote(player_id)
  ...
  def fetch_team_history(player_id)
    id_sql = conn.quote(player_id)
  ...
  def fetch_salary_book_yearly(player_id)
    id_sql = conn.quote(player_id)
  ...
  def fetch_contract_chronology(player_id)
    id_sql = conn.quote(player_id)
  ...
  def fetch_contract_versions(player_id)
    id_sql = conn.quote(player_id)
  ...
  def fetch_salaries(contract_id:, version_number:)
  ...
  def fetch_protections(contract_id:, version_number:)
    contract_sql = conn.quote(contract_id)
    version_sql = conn.quote(version_number)
  ...
  def fetch_protection_conditions(contract_id:, version_number:)
    contract_sql = conn.quote(contract_id)
    version_sql = conn.quote(version_number)
  ...
  def fetch_bonuses(contract_id:, version_number:)
    contract_sql = conn.quote(contract_id)
    version_sql = conn.quote(version_number)
  ...
  def fetch_bonus_maximums(contract_id:, version_number:)
    contract_sql = conn.quote(contract_id)
    version_sql = conn.quote(version_number)
  ...
  def fetch_payment_schedules(contract_id:, version_number:)
    contract_sql = conn.quote(contract_id)
    version_sql = conn.quote(version_number)
  ...
  def fetch_ledger_entries(player_id)
    id_sql = conn.quote(player_id)
