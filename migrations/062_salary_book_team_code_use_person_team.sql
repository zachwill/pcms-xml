-- 062_salary_book_team_code_use_person_team.sql
--
-- Fix team assignment in pcms.salary_book_warehouse.
--
-- team_code should reflect the player's current team (pcms.people.team_code),
-- not the team that originally signed the contract (contract_team_code).
...
BEGIN
  ...
  ac AS (
    ...
    SELECT
      ...
      CASE
        ...
        WHEN st.min_contract_code = '1OR2' THEN true
        WHEN st.min_contract_code IN ('NO', '3PLS') THEN false
        ELSE NULL
  ...
  SELECT
    ...
    -- team_code should reflect the player's *current* team (pcms.people.team_code).
    -- contract_team_code remains the contract/signing team for metadata.
    COALESCE(p.team_code, ac.team_code) AS team_code,
...
-- The refresh function uses a short lock_timeout internally, so take the lock
-- up-front to avoid intermittent failures on busy DBs.
LOCK TABLE pcms.salary_book_warehouse IN ACCESS EXCLUSIVE MODE;
