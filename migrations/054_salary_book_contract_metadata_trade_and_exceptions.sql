-- 054_salary_book_contract_metadata_trade_and_exceptions.sql
--
-- Add contract metadata needed for tool UIs (Salary Book / Trade tooling):
-- - Contract type (code + label) from lk_contract_types
-- - Signed method (Bird/MLE/BAE/minimum/etc) from lk_signed_methods
-- - Exception type (NTMLE/BAE/etc) via contracts.team_exception_id → team_exceptions → lk_exception_types
-- - Minimum contract classification (1OR2/3PLS/NO) via signing transaction → lk_min_contracts
-- - Trade restriction (EX6MO/DEC15/etc) from contract_versions.version_json → lk_trade_restrictions
--
-- Also change the "primary contract per player" selection to pick the most
-- recently signed APPR/FUTR contract (by contracts.signing_date), so rookies
-- with extensions surface as RSEX/VETEX/etc instead of RKSC.
--
-- Implementation notes:
-- - Warehouse stores BOTH codes + labels (no UI joins needed).
-- - Refresh remains TRUNCATE/INSERT and then recomputes pct_cap percentiles.

BEGIN;

ALTER TABLE pcms.salary_book_warehouse
  ADD COLUMN IF NOT EXISTS contract_type_code text,
  ADD COLUMN IF NOT EXISTS contract_type_lookup_value text,

  -- How the contract was signed (Bird/MLE/BAE/minimum/etc)
  ADD COLUMN IF NOT EXISTS signed_method_code text,
  ADD COLUMN IF NOT EXISTS signed_method_lookup_value text,

  -- If signed using a specific exception instance (e.g. NTMLE), capture it.
  ADD COLUMN IF NOT EXISTS team_exception_id integer,
  ADD COLUMN IF NOT EXISTS exception_type_code text,
  ADD COLUMN IF NOT EXISTS exception_type_lookup_value text,

  -- Minimum contract classification from the signing transaction (1OR2/3PLS/NO)
  ADD COLUMN IF NOT EXISTS min_contract_code text,
  ADD COLUMN IF NOT EXISTS min_contract_lookup_value text,

  -- Trade restrictions (EX6MO/DEC15/etc)
  ADD COLUMN IF NOT EXISTS trade_restriction_code text,
  ADD COLUMN IF NOT EXISTS trade_restriction_lookup_value text,
  ADD COLUMN IF NOT EXISTS trade_restriction_end_date date,
  ADD COLUMN IF NOT EXISTS is_trade_restricted_now boolean;
